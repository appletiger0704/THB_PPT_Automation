...
# 圖片儲存位置
img_save_path = r"C:\Users\user\Desktop\ppt_自動化"
...
#==============================================================================
print(f"本局-{now.year}{now.month}{yesterday.day} 未來三日天氣分析報告.pptx")

try:
    prs = Presentation(f"本局-{now.year}{now.month}{yesterday.day} 未來三日天氣分析報告.pptx")
except Exception as e:
    print(e)
...
first_page_pattern = r"\d{3}年\d{1,2}月\d{1,2}日 \d{2}:\d{2}時"
...
update_date(second_slide, Satellite_pattern, f"{now.month}月{now.day}日 06:00 衛星雲圖", RGBColor(12,51,115), 18)
